...
def adaptive_hill_climbing(coil_1, r1_turn, coil_2, r2_turn, d):
    ...
    while (np.array_equal(coil_1, coil_1q)
        ...
        for ind in range(len(coil_1) + len(coil_2)):
            ...
            # probability of mutation for a turn
            r = np.random.random()

            if r < rm and ind // len(coil_1) == 0 and ind != 0 and (ind + 1) != len(coil_1):
            ...
            elif r < rm and ind // len(coil_1) == 1 and ind != len(coil_1) and ind != (len(coil_2) + len(coil_1) - 1):
...
def launch(iterations, coil_t, rt_turn, coil_r, rr_turn,  d):
    ...
    for _ in range(iterations):
        ...
        coil_t1, coil_r1, k, allm, badm, goodm = adaptive_hill_climbing(
...
def main():
    ...
    if not FLAG_RUN_MULTIITER:
        ...
        --------------------------------------------------------------------
        '''
        coil_t, coil_r, k, allm, badm, goodm = adaptive_hill_climbing(coil_1=coil_t, r1_turn=r_turn,
